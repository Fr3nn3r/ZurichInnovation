# This script is an analysis tool designed to gather metrics about datasets that
# have been processed by other scripts in the workflow. It specifically focuses on
# analyzing the 'context files' (JSON files) generated by scripts like
# `generate_context.py`.
#
# The primary purpose of this script is to provide insights into the size and
# complexity of the datasets, which can be useful for understanding costs,
# processing times, and potential performance bottlenecks.
#
# The script's main functionalities are:
# 1.  **Context File Analysis**: It scans a specified 'output' directory for all
#     JSON files ending in '-context.json'.
#
# 2.  **Folder Size Calculation**: For each context file found, it determines the
#     corresponding original data folder (by parsing the filename). It then
#     calculates the total size (in kilobytes) of all the files within that
#     original data folder, providing a measure of the raw dataset size.
#
# 3.  **Token Estimation**: It reads the content of each JSON context file and
#     estimates the number of tokens that the content would consume if processed
#     by a large language model. This is done using the `tiktoken` library, which
#     is the standard for OpenAI models, providing a cost and complexity metric.
#
# 4.  **Excel Reporting**: The analysis results (folder name, folder size, and
#     estimated tokens) for each dataset are compiled and written to an Excel
#     spreadsheet.
#
# 5.  **Append and Update Logic**: The script is designed to be non-destructive.
#     If the specified Excel file already exists, it reads the existing data and
#     appends the new analysis results. It also includes logic to drop duplicate
#     entries based on the folder name, ensuring that running the analysis again
#     updates the data rather than creating redundant rows.
#
# Usage:
#   python src/analyze_output.py /path/to/output_folder /path/to/data_root /path/to/report.xlsx
